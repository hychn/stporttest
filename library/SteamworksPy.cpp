...
public:
    ...
    void FindLobbies()
    {
        ...
        //SteamMatchmaking()->AddRequestLobbyListStringFilter('','', 0)
    ...
    void OnLobbyMatchList(LobbyMatchList_t* pLobbyMatchList, bool bIOFailure)
    {
        ...
        for (int i = 0; i < pLobbyMatchList->m_nLobbiesMatching; i++) {
            CSteamID cid = SteamMatchmaking()->GetLobbyByIndex(i);
            const char* name = SteamMatchmaking()->GetLobbyData(cid, "name");
            printf("%s \n", name);
        }
...
/*
    //Start Server
    //??? Not in exampleSteamNetworkingUtils()->InitRelayNetworkAccess();
    ...
    // StartClient
    public ulong ConnectToSteamID; //??? What is this ???
    CSteamID serverUser = new CSteamID(ConnectToSteamID);
    SteamNetworkingUtils()->InitRelayNetworkAccess();
    SteamNetworkingIdentity smi = new SteamNetworkingIdentity();
    smi.SetSteamID(serverUser);
    ISteamNetworkingSockets connection = SteamNetworkingSockets.ConnectP2P(ref smi, 0, options.Length, options);
    ...
    //SEND
    int sendFlag = 0 //Constants.k_nSteamNetworkingSend_Unreliable;
