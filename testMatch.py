...
x = steamworks.Friends.GetPlayerName()
print( str(x, encoding='utf-8') )
L = steamworks.Friends.GetFriendCount()
L = 0
for i in range(L):
    id = steamworks.Friends.GetFriendByIndex(i)
    name = steamworks.Friends.GetFriendPersonaName(id)
    print(name)
    #print( str(name, encoding='utf-8') )

steamworks.Matchmaking.CreateLobby(0, 4)
